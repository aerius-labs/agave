...
fn execute_test_entry(test_entry: SvmTestEntry) {
    let mut mock_bank: MockBankCallback = MockBankCallback::default();

    for (name, slot) in &test_entry.initial_programs {
        deploy_program(name.to_string(), *slot, &mut mock_bank);
    }
    ...
    batch_processor.fill_missing_sysvar_cache_entries(&mock_bank.iavl);
    ...
    for (pubkey, expected_account_data) in test_entry.final_accounts.iter() {
        ...
        println!("ACTUAL: {} : {:?}", pubkey, actual_account_data);
        assert_eq!(
